...
//	display.drawRect(posX, posY, 32, 32, GxEPD_BLACK);
  ...
  display.fillRect(posX, posY + 21, 21, 21, GxEPD_WHITE);
  if (beeper.isWithZerotage()){
		display.drawInvertedBitmap(posX, posY + 21 , zerotageOn, 21, 21, GxEPD_BLACK); 
  } else {
		display.drawInvertedBitmap(posX, posY + 21 , zerotageOff, 21, 21, GxEPD_BLACK); 
  }
}
